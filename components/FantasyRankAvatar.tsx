 ...
 * Usa immagini PNG da public/avatars/ con fallback SVG se non disponibili
 * Mapping: rune -> scout, scroll -> analyst, crystal -> strategist, shield -> captain, helm -> commander, crown -> legend
 * Il parametro color viene usato come overlay/border per mantenere coerenza visiva
 */
export default function FantasyRankAvatar({ icon, color, size = 64 }: FantasyRankAvatarProps) {
  ...
  // SVG fallback quando l'immagine non è disponibile
  ...
  // Se l'immagine ha dato errore o non è disponibile, usa SVG fallback
  if (imageError) {
    return (
